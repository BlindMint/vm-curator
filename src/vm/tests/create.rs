...
fn test_write_vm_metadata_uses_vm_foundry_filename() {
    ...
    assert!(
        ...
        "metadata file should use vm-foundry.toml"
    );
...
fn test_update_network_in_script_preserves_case_syntax_and_usb_lines() {
    ...
    assert!(
        updated.contains("    --cdrom)\n"),
        "missing --cdrom case:\n{}",
        updated
    );
    assert!(
        updated.contains("        ;;\n    --cdrom)"),
        "missing case terminator before --cdrom:\n{}",
        updated
    );
    assert!(
        updated.contains("-usb"),
        "USB args should be preserved:\n{}",
        updated
    );
    assert!(
        updated.contains("-device usb-tablet"),
        "USB tablet should be preserved:\n{}",
        updated
    );
    assert!(
        updated.contains("-netdev bridge,id=net0,br=virbr1"),
        "bridge config should be applied:\n{}",
        updated
    );
